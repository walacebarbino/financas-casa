...
with st.sidebar:
    st.header("🗓️ Filtros")
    meses = df['MÊS'].unique()
    mes_ref = st.selectbox("Selecione o Mês:", meses, index=len(meses)-1)
...
# Lógica para bater com R$ 3.313,22
...
fig_pie = go.Figure(data=[go.Pie(labels=dist_data.index, values=dist_data.values, hole=.3, textinfo='label+percent')])
...
with st.expander("📝 Editar ou Excluir Linha"):
    ...
    with col_ed2:
        ...
        if st.button("💾 Salvar Alteração"):
            sheet.update_cell(int(linha_idx) + 2, 3, novo_status.upper())
